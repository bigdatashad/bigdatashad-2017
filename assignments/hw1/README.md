...
Важно: правильный результат метрики удовлетворяет 2м условиям: 1) правильная величина метрики 2) получена системой проверки в 9 утра.
Т.е. дни, обсчитанные ретроспективно, не учитываются. В интерфейсе Zwitter на странице Diagnostics самые важные элементы -- диагональные.
...
Для самопроверки вы можете использовать страницу по адресу `http://hadoop2-00.yandex.ru:8888/ysda/hw1`
(пункт меню 'HW 1, 2' --> 'Preview & Submit'), где
...
(структура ответа и типы возвращаемых значений), и произведена отрисовка ответа. После нажания на "Submit" решение будет
включено в список для ежедневной провеки. Эту процедуру достаточно проделать один раз. Однако всякий раз после изменения вывода,
например, после добавления новой метрики в выдачу, можно делать "Preview" для проверки правильности формата.
...
Лог за день считается готовым только тогда, когда появляется лог (директория лога) за следующий день.
Это важно проверять, не стоит делать расчеты на неполном логе.
